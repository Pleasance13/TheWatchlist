...
function applyAdvancedFilters(items){
 ...
 if(state.interestUsers?.length&&state.interestLevel)a=a.filter(m=>state.interestUsers.some(p=>{
   let answer=null;
 ...
 if(state.rewatchStatus)a=a.filter(m=>Boolean(m.setToRewatch)=== (state.rewatchStatus==="yes"));
