...
function detail(){
 ...
 <section class="detail ${m.watched?"history-detail":""}">
   ...
   <div>
    ...
    <div class="people-strip"><div class="people-strip-label">${m.watched?"WHO WATCHED":"RESPONSES"}</div>${m.watched?stack((m.watchedBy||[]).map(n=>[n,"watched","Watched"])):stack(voterEntries(m).map(([n,color])=>{const mine=n==="Josh"?currentVote(m.id):null;return [n,color,mine?responseLabel(mine):({green:"Interested",yellow:"I’d Watch",red:"Not Interested"})[color]||"No response"]}))}</div>
