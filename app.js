...
function detail(){
 ...
 <section class="detail ${m.watched?"history-detail":""}">
   ...
   <div>
    ...
    ${state.showNote?suggestionNoteMarkup(m):""}
...
function suggestionNoteMarkup(m){const who=m.suggestedBy||m.addedBy||(m.note?"Josh":"Unknown");const editable=who===currentUser;return '<div class="detail-note"><div class="detail-note-label"><span class="avatar">'+escapeHtml(who.slice(0,1).toUpperCase())+'</span><span>Suggested by '+escapeHtml(who)+'</span>'+(editable?'<button class="ghost edit-suggestion-note" onclick="editSuggestionNote(\''+m.id+'\')">'+(m.note?"Edit note":"Add note")+'</button>':"")+'</div>'+(m.note?'<div class="detail-note-text">'+escapeHtml(m.note)+'</div>':"")+'</div>'}
...
window.editSuggestionNote=id=>{const m=movies.find(x=>x.id===id);if(!m||(m.suggestedBy||m.addedBy||"Josh")!==currentUser)return;const note=prompt(m.note?"Edit your suggestion note:":"Add a note to your suggestion:",m.note||"");if(note===null)return;m.note=note.trim();m.suggestedBy=m.suggestedBy||m.addedBy||currentUser;m.addedBy=m.addedBy||currentUser;try{if(m.id.startsWith("tmdb-"))localStorage.setItem("watchlist-added-movies",JSON.stringify(movies.filter(x=>x.id.startsWith("tmdb-")).map(x=>({...x,score:0}))));else{let notes={};try{notes=JSON.parse(localStorage.getItem("watchlist-suggestion-notes")||"{}")}catch(error){}notes[m.id]=m.note;localStorage.setItem("watchlist-suggestion-notes",JSON.stringify(notes));}}catch(error){}render()};
