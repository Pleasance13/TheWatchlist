...
const state={nav:"watchlist",view:"list",search:"",filter:"all",posterSize:175,showSynopsis:true,showRatings:false,showNote:true,showTrailer:false,detail:null,showFilters:false,votes:{},expandedPeople:{}};
...
function caseArticle(m,extra=""){return `<article class="vhs ${extra}" data-vhs="${m.id}" data-movie-id="${m.id}" onclick="toggleCase(event,this)"><div class="vhs-stage">${caseFaces(m,backContent(m))}</div><div class="grid-title" onclick="event.stopPropagation();openMovie('${m.id}')">${m.title}</div><div class="grid-meta" onclick="event.stopPropagation();openMovie('${m.id}')">${m.year} · ${m.genre}</div>${responseButtons(m,true)}<div class="hint">Click the case to flip · click the title for details</div></article>`}
...
function people(){
 const people=["Josh","Sarah","Mike","Alex"];
 return `<div class="hero"><div><div class="eyebrow">THE SERVER</div><h1>People</h1><p class="sub">Who's in the group and what they're interested in.</p></div></div><div class="people-list">${people.map(p=>{
   const seen=movies.filter(m=>m.seen.includes(p));
   const interests=movies.filter(m=>m.voters.some(([n])=>n===p));
   const suggested=movies.filter(m=>p==="Josh"&&m.note);
   const expanded=state.expandedPeople[p];
   return `<section class="person-card ${expanded?"expanded":""}">
     <button class="person-summary" onclick="togglePerson('${p}')" aria-expanded="${expanded?"true":"false"}">
       <div class="person-main"><div class="avatar">${p.slice(0,2)}</div><div><strong>${p}</strong><span>${interests.length} active interests · ${seen.length} seen before</span></div></div>
       <span class="person-chevron">${expanded?"⌃":"⌄"}</span>
     </button>
     ${expanded?`<div class="person-details">
       <div class="person-section"><div class="person-section-label">INTERESTED IN WATCHING</div>${interests.length?<div class="person-movies">${interests.map(m=>{const v=m.voters.find(([n])=>n===p);return `<div class="person-movie"><span>${m.title}</span><span class="person-interest ${v[1]}">${v[1]==="green"?"Interested":v[1]==="yellow"?"I'd Watch":"Not Interested"}</span></div>`}).join("")}</div>:`<div class="person-empty">No interest responses yet.</div>`}</div>
       <div class="person-section"><div class="person-section-label">SEEN</div>${seen.length?<div class="person-movies">${seen.map(m=>`<div class="person-movie"><span>${m.title}</span><span class="person-seen">Seen</span></div>`).join("")} </div>:`<div class="person-empty">No movies marked as seen.</div>`}</div>
       ${suggested.length?`<div class="person-section"><div class="person-section-label">SUGGESTED</div><div class="person-movies">${suggested.map(m=>`<div class="person-movie"><span>${m.title}</span><span class="person-seen">Suggestion</span></div>`).join("")}</div></div>`:""}
     </div>`:""}
   </section>
 }).join("")}</div>`;
}
