...
function listView(items){return `<div class="list">${items.map((m,i)=>{const mine=currentVote(m.id);return `<article class="row ${mine==="no"?"not-interested-row":""}"><div class="rank">#${i+1}</div><div class="list-vhs" aria-hidden="true"><div class="list-vhs-label">${m.title}<span>${m.year}</span></div><div class="list-vhs-spine"></div></div><div onclick="openMovie('${m.id}')" style="cursor:pointer"><div class="title">${m.title}${mine?` <span class="response-chip ${mine}">${responseLabel(mine)}</span>`:""}</div><div class="meta">${m.year} · ${m.genre} · ${m.director}</div></div><div>${stack(voterEntries(m))}</div><div class="seen">${m.seen.includes("Josh")?"◉ Seen":"○ Not seen"}</div></article>`}).join("")}</div>`}
