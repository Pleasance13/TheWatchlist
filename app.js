...
const state={nav:"watchlist",view:"list",search:"",filter:"all",posterSize:2,showSynopsis:true,showRatings:false,showNote:true,showTrailer:false,detail:null,showFilters:false,genreFilters:[],yearFrom:"",yearTo:"",interestUser:"",interestLevel:"",watchedWith:"",suggestedBy:"",votes:{},addMovieOpen:false,addMovieQuery:"",addMovieResults:[],addMovieSelection:null,addMovieLoading:false,addMovieError:"",attendanceOpen:false,attendanceMovieId:null,attendanceSelected:[],assetEditorOpen:false,assetMovieId:null,assetLoading:false,assetError:"",assetSections:{frontLogo:true,detailLogo:true,frontImage:true,backStill:true},assetLanguageGroups:{},assetPreviewFlipped:false};
...
function advancedFilterPanel(){const genres=genreList();const selected=state.genreFilters||[];return \`<div class="filter-panel"><div class="filter-presets">\${[["all","All movies"],["unwatched","Unwatched"],["nobody seen","Nobody has seen"],["I haven't seen","I haven't seen"],["not interested","I'm not interested"]].map(([f,l])=>\`<button class="\${state.filter===f?"active":""}" data-filter="\${f}" onclick="setFilter(this.dataset.filter)">\${l}</button>\`).join("")}</div><div class="filter-advanced"><label class="filter-field"><span>Genre (multiple)</span><details class="filter-dropdown"><summary>\${selected.length?selected.join(", "):"Any genre"}</summary><div class="filter-options">\${genres.map(g=>\`<label><input type="checkbox" \${selected.includes(g)?"checked":""} onchange="toggleGenre(this.value,this.checked)" value="\${g.replace(/&/g,"&amp;").replace(/"/g,"&quot;")}"> \${g}</label>\`).join("")}</div></details></label><div class="filter-field"><span>Year range</span><div class="year-range"><input type="number" placeholder="From" min="1888" max="2100" value="\${state.yearFrom||""}" onchange="setYear('from',this.value)"><input type="number" placeholder="To" min="1888" max="2100" value="\${state.yearTo||""}" onchange="setYear('to',this.value)"></div></div><label class="filter-field"><span>Interest level</span><div class="filter-pair"><select onchange="setInterestUser(this.value)"><option value="">Any user</option>\${serverUsers.map(p=>\`<option \${state.interestUser===p?"selected":""}>\${p}</option>\`).join("")}</select><select onchange="setInterestLevel(this.value)"><option value="">Any level</option>\${[["must","Must Watch"],["interested","Interested"],["watch","I'd Watch"],["no","Not Interested"]].map(([v,l])=>\`<option value="\${v}" \${state.interestLevel===v?"selected":""}>\${l}</option>\`).join("")}</select></div></label><label class="filter-field"><span>Previously watched with</span><select onchange="setWatchedWith(this.value)"><option value="">Anyone / no filter</option><option value="any" \${state.watchedWith==="any"?"selected":""}>Any group history</option>\${serverUsers.map(p=>\`<option \${state.watchedWith===p?"selected":""}>\${p}</option>\`).join("")}</select></label><label class="filter-field"><span>Suggested by</span><select onchange="setSuggestedBy(this.value)"><option value="">Anyone</option>\${serverUsers.map(p=>\`<option \${state.suggestedBy===p?"selected":""}>\${p}</option>\`).join("")}</select></label><button class="ghost filter-reset" onclick="resetAdvancedFilters()">Reset advanced filters</button></div></div>\`}
function toolbar(){return \`<div class="toolbar"><input id="search" class="search" placeholder="Search movies..." value="\${state.search}"><button class="icon \${state.view==="list"?"active":""}" onclick="setView('list')">☷ <span>List</span></button><button class="icon \${state.view==="grid"?"active":""}" onclick="setView('grid')"><svg class="toolbar-icon-svg" viewBox="0 0 18 18" aria-hidden="true">\${Array.from({length:9},(_,i)=>\`<rect x="\${(i%3)*6+1}" y="\${Math.floor(i/3)*6+1}" width="4" height="4" rx=".7"></rect>\`).join("")}</svg><span>Grid</span></button><button class="ghost filter-button" onclick="toggleFilters()"><svg class="filter-icon-svg" viewBox="0 0 20 20" aria-hidden="true"><path d="M2.5 4.5h15L12 10.5v4.1l-3.9 1.9v-6Z"></path></svg><span>Filters</span></button>\${state.view==="grid"?\`<input class="range" id="sizeRange" type="range" min="1" max="4" step="1" value="\${state.posterSize}" aria-label="Poster size">\`:""}</div>\${state.showFilters?advancedFilterPanel():""}\`}
...
function applyAdvancedFilters(items){let a=items.slice();const genres=state.genreFilters||[];if(genres.length)a=a.filter(m=>genres.every(g=>(m.genre||"").split(/\s*[·,/&]\s*/).map(x=>x.trim()).includes(g)));if(state.yearFrom)a=a.filter(m=>Number(m.year)>=Number(state.yearFrom));if(state.yearTo)a=a.filter(m=>Number(m.year)<=Number(state.yearTo));if(state.interestUser&&state.interestLevel)a=a.filter(m=>{const p=state.interestUser;const raw=p==="Josh"?currentVote(m.id):m.voters.find(([n])=>n===p)?.[1];if(p==="Josh")return raw===state.interestLevel;const color=raw;return state.interestLevel==="must"||state.interestLevel==="interested"?color==="green":state.interestLevel==="watch"?color==="yellow":color==="red"});if(state.watchedWith){a=a.filter(m=>{const who=m.watchedBy||[];return state.watchedWith==="any"?who.length>0:who.includes(state.watchedWith)})}if(state.suggestedBy)a=a.filter(m=>(m.suggestedBy||m.addedBy||(m.note?"Josh":""))===state.suggestedBy);return a}
function watchlist(){
...
window.setYear=(which,value)=>{state[which==="from"?"yearFrom":"yearTo"]=value;render()};
window.setInterestUser=x=>{state.interestUser=x;render()};
window.setInterestLevel=x=>{state.interestLevel=x;render()};
window.setWatchedWith=x=>{state.watchedWith=x;render()};
window.setSuggestedBy=x=>{state.suggestedBy=x;render()};
window.resetAdvancedFilters=()=>{state.genreFilters=[];state.yearFrom="";state.yearTo="";state.interestUser="";state.interestLevel="";state.watchedWith="";state.suggestedBy="";state.filter="all";render()};
