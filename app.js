...
function listVhs(m){return `<div class="list-vhs" aria-hidden="true"><div class="list-vhs-stage">${caseFaces(m,backContent(m),"list-vhs-inner")}</div></div>`}\nfunction listView(items){return `<div class="list">${items.map((m,i)=>{const mine=currentVote(m.id);return `<article class="row ${mine==="no"?"not-interested-row":""}"><div class="rank">#${i+1}</div>${listVhs(m)}<div onclick="openMovie('${m.id}')" style="cursor:pointer"><div class="title">${m.title}${mine?` <span class="response-chip ${mine}">${responseLabel(mine)}</span>`:""}</div><div class="meta">${m.year} · ${m.genre} · ${m.director}</div></div><div>${stack(voterEntries(m))}</div><div class="seen">${m.seen.includes("Josh")?"◉ Seen":"○ Not seen"}</div></article>`}).join("")}</div>`}
...
function bindVhsTilt(){document.querySelectorAll("[data-vhs]").forEach(card=>{card.addEventListener("pointermove",e=>{const r=card.getBoundingClientRect(),x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;card.style.setProperty("--tilt-x",`${(-y*28).toFixed(2)}deg`);card.style.setProperty("--tilt-y",`${(x*38).toFixed(2)}deg`)});card.addEventListener("pointerleave",()=>{card.style.setProperty("--tilt-x","0deg");card.style.setProperty("--tilt-y","0deg")})})}
