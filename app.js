...
function personVote(m,p){if(p==="Josh"){const hasSavedVote=Object.prototype.hasOwnProperty.call(state.votes,m.id);const vote=hasSavedVote?currentVote(m.id):(m.voters||[]).find(([name])=>name===p)?.[1];return vote?({must:"green",interested:"green",watch:"yellow",no:"red"}[vote]||null):null}const v=(m.voters||[]).find(([n])=>n===p);return v?v[1]:null}
...
const row=(m,label,cls,showWatchers=false)=>'<button type="button" class="person-movie" onclick="event.stopPropagation();openMovie(this.dataset.id)" data-id="'+m.id+'"><span>'+m.title+'</span><span class="'+cls+'">'+label+'</span>'+(showWatchers?stack((m.watchedBy||[]).map(n=>[n,"green","Watched"])):"")+'</button>';
