...
function advancedFilterPanel(historyMode=false){
 ...
 <div class="filter-control"><details class="filter-dropdown" data-filter-id="year"><summary>Release year (range) ▾</summary><div class="filter-menu year-range-menu"><div class="year-range-values"><span>From: <b id="year-from-label">${state.yearFrom||minYear}</b></span><span>To: <b id="year-to-label">${state.yearTo||maxYear}</b></span></div><div class="dual-range"><input aria-label="From year" type="range" min="${minYear}" max="${state.yearTo||maxYear}" step="1" value="${state.yearFrom||minYear}" onchange="setYear('from',this.value)"><input aria-label="To year" type="range" min="${state.yearFrom||minYear}" max="${maxYear}" step="1" value="${state.yearTo||maxYear}" onchange="setYear('to',this.value)"></div><small>Available years: ${minYear}–${maxYear}</small></div></details>${filterActive("yearFrom")||filterActive("yearTo")?'<button class="filter-clear" title="Clear release year range" aria-label="Clear release year range" onclick="clearOneFilter(\'yearFrom\')">×</button>':""}</div>
...
window.setYear=(which,value)=>{const years=movies.map(m=>Number(m.year)).filter(y=>Number.isFinite(y)&&y>0),min=years.length?Math.min(...years):1888,max=years.length?Math.max(...years):new Date().getFullYear();let n=Math.max(min,Math.min(max,Number(value)));if(which==="from"){const upper=Number(state.yearTo)||max;n=Math.min(n,upper);state.yearFrom=String(n);if(state.yearTo&&Number(state.yearTo)<n)state.yearTo=String(n)}else{const lower=Number(state.yearFrom)||min;n=Math.max(n,lower);state.yearTo=String(n)}render()};
