...
function header(){return `<header class="topbar"><button class="brand" onclick="setNav('watchlist');setView('list')" aria-label="Go to Watchlist"><img class="brand-logo" src="thewatchlist-logo.svg" alt="The Watchlist"></button><nav class="nav">${["watchlist","history","people","settings"].map(x=>`<button class="${state.nav===x?"active":""}" onclick="setNav('${x}')">${x[0].toUpperCase()+x.slice(1)}</button>`).join("")}</nav><div class="user"><div class="avatar">J</div><span>Josh</span></div></header>`}
...
function people(){return `<div class="hero"><div><div class="eyebrow">THE SERVER</div><h1>People</h1><p class="sub">Who's in the group and what they're interested in.</p></div></div><div class="settings">${["Josh","Sarah","Dan","Sam","Alex"].map(p=>{const unseen=movies.filter(m=>!m.seen.includes(p));const relevant=unseen.filter(m=>{const v=personVote(m,p);return !v||v!=="red"});const interested=relevant.filter(m=>personVote(m,p));const suggested=movies.filter(m=>m.suggestedBy===p||m.addedBy===p||(p==="Josh"&&!m.suggestedBy&&!m.addedBy&&Boolean(m.note)));return `<div class="setting person-card" data-person="${p}" onclick="togglePerson('${p}')" style="cursor:pointer"><div style="display:flex;gap:13px;align-items:center"><div class="avatar">${p.slice(0,2)}</div><div><strong>${p}</strong><span>Interested in ${interested.length} · Hasn't seen ${relevant.length} · Suggested ${suggested.length}</span></div></div></div>`}).join("")}</div>`}
...
function render(){const openFilterIds=[...document.querySelectorAll(".filter-dropdown[open][data-filter-id]")].map(el=>el.dataset.filterId);app.innerHTML=header()+`<main class="content">${state.nav==="watchlist"?watchlist():state.nav==="history"?history():state.nav==="people"?people():state.nav==="settings"?settings():detail()}</main>`+addMovieModal()+attendanceModal()+assetEditor()+removeMovieModal();openFilterIds.forEach(id=>{const el=[...document.querySelectorAll(".filter-dropdown[data-filter-id]")].find(x=>x.dataset.filterId===id);if(el)el.open=true});bindLiveInputs();bindVhsTilt()}
