...
const watched=movies.filter(m=>(m.watchedBy||[]).includes(p)||m.seen.includes(p));
const together=movies.filter(m=>(m.watchedBy||[]).includes(p)&&(m.watchedBy||[]).includes(currentUser));
