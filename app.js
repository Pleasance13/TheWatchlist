...
function header(){return `<header class="topbar"><div class="header-prism" aria-hidden="true"><img src="thewatchlist-prism.svg" alt=""></div><button class="brand" onclick="setNav('watchlist');setView('list')" aria-label="Go to Watchlist"><img class="brand-logo" src="tracking-logo.svg" alt="The Watchlist"></button><nav class="nav">${["watchlist","history","people","settings"].map(x=>`<button class="${state.nav===x?"active":""}" onclick="setNav('${x}')"><span>${x[0].toUpperCase()+x.slice(1)}</span></button>`).join("")}</nav><div class="user"><div class="avatar">J</div><span>Josh</span></div></header>`}
...
function suggestionNoteMarkup(m){const who=m.suggestedBy||m.addedBy||(m.note?"Josh":"Unknown");const editable=who===currentUser;return '<div class="detail-note '+(!m.note?'detail-note-empty':'')+'"><div class="detail-note-label"><span class="avatar">'+escapeHtml(who.slice(0,1).toUpperCase())+'</span><span>Suggested by '+escapeHtml(who)+'</span></div>'+(m.note?'<div class="detail-note-text">'+escapeHtml(m.note)+'</div>':"")+(editable?'<div class="detail-note-actions"><button class="edit-suggestion-note" onclick="editSuggestionNote(\''+m.id+'\')">'+(m.note?"Edit note":"Add note")+'</button></div>':"")+'</div>'}
