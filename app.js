...
function advancedFilterPanel(historyMode=false){
 ...
 const multi=(items,arr,handler)=>items.map(p=>`<label><input type="checkbox" value="${p}" ${arr.includes(p)?"checked":""} onchange="${handler}(this.value,this.checked)"> ${p===currentUser?p+" (you)":p}</label>`).join("");
 return `<div class="filter-panel">
 <button class="filter-all filter-option-button" onclick="clearAllFilters()">All movies</button>
 ...
 <div class="filter-control"><details class="filter-dropdown" data-filter-id="rewatch"><summary> ▾</summary><div class="filter-menu"><div class="filter-options rewatch-status-options"><label><input type="radio" name="rewatch-status" value="yes" ${state.rewatchStatus==="yes"?"checked":""} onchange="setRewatchStatus(this.value)"> Yes</label><label><input type="radio" name="rewatch-status" value="no" ${state.rewatchStatus==="no"?"checked":""} onchange="setRewatchStatus(this.value)"> No</label></div></div></details>${filterChip("Set to rewatch","rewatchStatus")}</div>
 </div>`
}
...
function listView(items,historyMode=false){return `<div class="list">${items.map((m,i)=>{const mine=currentVote(m.id),rank=rankOf(m);return `<article class="row ${mine==="no"?"not-interested-row":""} ${historyMode?"history-row":""}" data-movie-id="${m.id}">${historyMode?"":`<div class="rank">${rank<=5?rankSticker(rank,"list-rank-sticker"):`#${rank}`}</div>`}<div class="list-vhs" aria-hidden="true"><div class="vhs-stage">${caseFaces(m,"","list-vhs-inner")}</div></div><div onclick="openMovie('${m.id}')" style="cursor:pointer"><div class="title">${m.title}</div><div class="meta">${m.year} · ${m.genre} · ${m.director}</div>${historyMode?"":responseButtons(m,true)}</div><div>${historyMode?stack((m.watchedBy||[]).map(n=>[n,"green"])):stack(voterEntries(m))}</div><div class="seen">${m.seen.includes("Josh")?"◉ Seen":"○ Not seen"}</div></article>`}).join("")}</div>`}
...
function render(){const openFilterIds=[...document.querySelectorAll(".filter-dropdown[open][data-filter-id]")].map(el=>el.dataset.filterId);app.innerHTML=header()+`<main class="content">${state.nav==="watchlist"?watchlist():state.nav==="history"?history():state.nav==="people"?people():state.nav==="settings"?settings():detail()}</main>`+addMovieModal()+attendanceModal()+assetEditor();openFilterIds.forEach(id=>{const el=[...document.querySelectorAll(".filter-dropdown[data-filter-id]")].find(x=>x.dataset.filterId===id);if(el)el.open=true});bindLiveInputs();bindVhsTilt()}
...
window.toggleInterestUser=(p,on)=>{state.interestUsers=on?[...new Set([...state.interestUsers,p])]:state.interestUsers.filter(x=>x!==p);render()};window.setInterestLevel=x=>{state.interestLevel=x;render()};
