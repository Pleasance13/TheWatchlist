...
function personVote(m,p){if(p==="Josh"){const mine=currentVote(m.id);return mine?({must:"green",interested:"green",watch:"yellow",no:"red"}[mine]||null):null}const v=m.voters.find(([n])=>n===p);return v?v[1]:null}
function people(){return `<div class="hero"><div><div class="eyebrow">THE SERVER</div><h1>People</h1><p class="sub">Reviews, watch history, and what everyone wants to see.</p></div></div><div class="settings">${["Josh","Sarah","Dan","Sam","Alex"].map(p=>{const unseen=movies.filter(m=>!m.seen.includes(p));const relevant=unseen.filter(m=>{const v=personVote(m,p);return !v||v!=="red"});const interested=relevant.filter(m=>personVote(m,p));const suggested=movies.filter(m=>m.suggestedBy===p||m.addedBy===p||(p==="Josh"&&!m.suggestedBy&&!m.addedBy&&Boolean(m.note)));const reviewed=movies.filter(m=>(movieReviews[m.id]||{})[p]);const watched=movies.filter(m=>(m.watchedBy||[]).includes(p));return `<div class="setting person-card" data-person="${p}" onclick="togglePerson('${p}')" style="cursor:pointer"><div style="display:flex;gap:13px;align-items:center"><div class="avatar">${p.slice(0,2)}</div><div><strong>${p}</strong><span>Interested in ${interested.length} · Hasn't seen ${relevant.length} · Watched ${watched.length} · Reviews ${reviewed.length}</span></div></div></div>`}).join("")}</div>`}
...
function detail(){
 ...
 <section class="detail ${m.watched?"history-detail":""}">
   ...
   <div>
    ...
    <div class="people-strip"><div class="people-strip-label">${m.watched?"WHO WATCHED":"RESPONSES"}</div>${m.watched?stack((m.watchedBy||[]).map(n=>[n,"green","Watched"])):stack(voterEntries(m).map(([n,color])=>[n,color,({green:"Interested",yellow:"I’d Watch",red:"Not Interested"})[color]||"No response"]))}</div>
...
function syncHeaderPrism(){const bar=document.querySelector(".topbar"),logo=document.querySelector(".brand-logo"),avatar=document.querySelector(".topbar .user");const prism=document.querySelector(".header-prism");if(!bar||!logo||!avatar||!prism)return;const b=bar.getBoundingClientRect(),l=logo.getBoundingClientRect(),u=avatar.getBoundingClientRect();const left=l.right-b.left-l.width*.139,right=u.left-b.left-24,top=l.top+l.height/2-b.top,height=l.height,width=Math.max(0,right-left);prism.style.setProperty("left",left+"px","important");prism.style.setProperty("right","auto","important");prism.style.top=top+"px";prism.style.setProperty("width",width+"px","important");prism.style.height=height+"px";prism.style.transform="translateY(-50%)";prism.style.setProperty("--prism-height",height+"px");}
...
window.confirmAddMovie=()=>{const d=state.addMovieSelection;if(!d?.tmdbId)return;const normalizeTitle=s=>String(s||"").trim().toLowerCase().replace(/[^a-z0-9]+/g," ");const duplicate=movies.some(m=>m.tmdbId===d.tmdbId||(normalizeTitle(m.title)===normalizeTitle(d.title)&&String(m.year)===String(d.year||"")));if(duplicate){state.addMovieError="Already in watchlist";return render()}const note=(document.querySelector("#addMovieNote")?.value||"").trim();const movie={id:"tmdb-"+d.tmdbId,title:d.title,year:d.year||"",genre:(d.genre||[]).join(" · "),director:(d.director||[]).join(", "),runtime:d.runtime?formatRuntime(d.runtime):"",rating:d.rating||"",score:0,seen:[],voters:[],synopsis:d.synopsis||"",note,warnings:[],watched:false,suggestedBy:currentUser,addedBy:currentUser,releaseDate:d.releaseDate||"",tmdbId:d.tmdbId,posterPath:d.posterPath||null,textlessPosterPath:d.textlessPosterPath||null,backdropPath:d.backdropPath||null,logoPath:d.logoPath||null};movies.push(movie);try{localStorage.setItem("watchlist-added-movies",JSON.stringify(movies.filter(m=>m.id.startsWith("tmdb-")).map(m=>({...m,score:0}))));}catch(error){}state.addMovieOpen=false;state.addMovieSelection=null;state.addMovieError="";render()};
...
const interests=notSeen.filter(m=>{const v=personVote(m,p);return v==="green"||v==="yellow"});
...
const row=(m,label,cls)=>'<button type="button" class="person-movie" onclick="event.stopPropagation();openMovie(this.dataset.id)" data-id="'+m.id+'"><span>'+m.title+'</span><span class="'+cls+'">'+label+'</span></button>';
...
details.innerHTML=section("INTERESTED IN WATCHING",interests.map(m=>row(m,personVote(m,p)==="green"?"Interested":"I’d Watch","person-interest "+personVote(m,p))),"No interest responses yet.")+section("MOVIES WATCHED",watched.map(m=>row(m,"Watched","person-seen")),"No watched movies recorded.")+(p===currentUser?"":section("WATCHED WITH YOU",together.map(m=>row(m,"Together","person-seen")),"No movies recorded as watched together."))+section("REVIEWS",reviewed.map(m=>row(m,(movieReviews[m.id][p].rating?movieReviews[m.id][p].rating+"/5 · ":"")+(movieReviews[m.id][p].review||"Review posted"),"person-seen")),"No reviews yet.")+(suggested.length?section("SUGGESTED",suggested.map(m=>row(m,"Suggestion","person-seen")),""):"");
